...
import { createComponentsTypingsInterface } from "./create-components-typings-interface";
...
export async function run(file = null) {
  ...
  const templateComponent = await createComponentsTypingsInterface(theme
    );
  if (process.send) {
    process.send(templateComponent);
 
  } else {
    process.stdout.write(templateComponent)
  }
}
